...
static bool
isMemberPointer(StringView MangledName, bool &Error)
{
    Error = false;
...
static bool
startsWithLocalScopePattern(StringView S)
{
    if (!S.consumeFront('?'))
        return false;
    if (S.size() < 2)
...
static bool
isCustomType(StringView S)
{
    return S[0] == '?';
}

static bool
isPointerType(StringView S)
{
...
static bool
isArrayType(StringView S)
{
    return S[0] == 'Y';
}

static bool
isFunctionType(StringView S)
{
...
Demangler::demangleSpecialTableSymbolNode(StringView &MangledName, SpecialIntrinsicKind K)
{
...
static NamedIdentifierNode *
synthesizeNamedIdentifier(ArenaAllocator &Arena, StringView Name)
{
...
synthesizeQualifiedName(ArenaAllocator &Arena, IdentifierNode *Identifier)
{
...
static QualifiedNameNode *
synthesizeQualifiedName(ArenaAllocator &Arena, StringView Name)
{
    NamedIdentifierNode *Id = synthesizeNamedIdentifier(Arena, Name);
    return synthesizeQualifiedName(Arena, Id);
}

static VariableSymbolNode *
synthesizeVariable(ArenaAllocator &Arena, TypeNode *Type, StringView VariableName)
{
...
VariableSymbolNode *
Demangler::demangleRttiBaseClassDescriptorNode(ArenaAllocator &Arena, StringView &MangledName)
{
...
FunctionSymbolNode *
Demangler::demangleInitFiniStub(StringView &MangledName, bool IsDestructor)
{
...
SymbolNode *
Demangler::demangleSpecialIntrinsic(StringView &MangledName)
{
...
Demangler::demangleStructorIdentifier(StringView &MangledName, bool IsDestructor)
{
...
IdentifierNode *
Demangler::demangleFunctionIdentifierCode(StringView &MangledName, FunctionIdentifierCodeGroup Group)
{
    // RetDec {
    if (!MangledName.empty())
    {
...
SymbolNode *
Demangler::demangleEncodedSymbol(StringView &MangledName, QualifiedNameNode *Name)
{
    // Read a variable.
    switch (MangledName.front())
    {
...
// Parser entry point.
SymbolNode *
Demangler::parse(StringView &MangledName)
{
...
TagTypeNode *
Demangler::parseTagUniqueName(StringView &MangledName)
{
...
VariableSymbolNode *
Demangler::demangleVariableEncoding(StringView &MangledName, StorageClass SC)
{
    VariableSymbolNode *VSN = Arena.alloc<VariableSymbolNode>();
...
Demangler::demangleNumber(StringView &MangledName)
{
...
uint64_t
Demangler::demangleUnsigned(StringView &MangledName)
{
...
int64_t
Demangler::demangleSigned(StringView &MangledName)
{
    bool IsNegative = false;
    uint64_t Number = 0;
    std::tie(Number, IsNegative) = demangleNumber(MangledName);
...
// Memorize it.
void
Demangler::memorizeString(StringView S)
{
...
NamedIdentifierNode *
Demangler::demangleBackRefName(StringView &MangledName)
{
...
void
Demangler::memorizeIdentifier(IdentifierNode *Identifier)
{
...
IdentifierNode *
Demangler::demangleTemplateInstantiationName(StringView &MangledName, NameBackrefBehavior NBB)
{
    assert(MangledName.startsWith("?$"));
    MangledName.consumeFront("?$");
...
NamedIdentifierNode *
Demangler::demangleSimpleName(StringView &MangledName, bool Memorize)
{
...
static bool
isRebasedHexDigit(char C)
{
    return (C >= 'A' && C <= 'P');
}

static uint8_t
rebasedHexDigitToNumber(char C)
{
...
uint8_t
Demangler::demangleCharLiteral(StringView &MangledName)
{
    if (!MangledName.startsWith('?'))
...
wchar_t
Demangler::demangleWcharLiteral(StringView &MangledName)
{
    uint8_t C1, C2;
...
static void
writeHexDigit(char *Buffer, uint8_t Digit)
{
    assert(Digit <= 15);
...
static void
outputHex(OutputStream &OS, unsigned C)
{
    if (C == 0)
    {
        OS << "\\x00";
...
static void
outputEscapedChar(OutputStream &OS, unsigned C)
{
    switch (C)
    {
    case '\'': // single quote
...
static unsigned
countTrailingNullBytes(const uint8_t *StringBytes, int Length)
{
...
static unsigned
countEmbeddedNulls(const uint8_t *StringBytes, unsigned Length)
{
    unsigned Result = 0;
...
static unsigned
guessCharByteSize(const uint8_t *StringBytes, unsigned NumChars, unsigned NumBytes)
{
    assert(NumBytes > 0);
...
static unsigned
decodeMultiByteChar(const uint8_t *StringBytes, unsigned CharIndex, unsigned CharBytes)
{
...
FunctionSymbolNode *
Demangler::demangleVcallThunkNode(StringView &MangledName)
{
...
StringView
Demangler::demangleSimpleString(StringView &MangledName, bool Memorize)
{
    StringView S;
    for (size_t i = 0; i < MangledName.size(); ++i)
    {
...
IdentifierNode *
Demangler::demangleUnqualifiedTypeName(StringView &MangledName, bool Memorize)
{
...
IdentifierNode *
Demangler::demangleUnqualifiedSymbolName(StringView &MangledName, NameBackrefBehavior NBB)
{
    if (startsWithDigit(MangledName))
        return demangleBackRefName(MangledName);
    if (MangledName.startsWith("?$"))
...
IdentifierNode *
Demangler::demangleNameScopePiece(StringView &MangledName)
{
    if (startsWithDigit(MangledName))
        return demangleBackRefName(MangledName);
...
static NodeArrayNode *
nodeListToNodeArray(ArenaAllocator &Arena, NodeList *Head, size_t Count)
{
...
QualifiedNameNode *
Demangler::demangleNameScopeChain(StringView &MangledName, IdentifierNode *UnqualifiedName)
{
...
FuncClass
Demangler::demangleFunctionClass(StringView &MangledName)
{
    switch (MangledName.popFront())
    {
    case '9':
...
StorageClass
Demangler::demangleVariableStorageClass(StringView &MangledName)
{
...
//                 ::= <type> <pointee-cvr-qualifiers> # pointers, references
TypeNode *
Demangler::demangleType(StringView &MangledName, QualifierMangleMode QMM)
{
...
bool
Demangler::demangleThrowSpecification(StringView &MangledName)
{
...
Demangler::demangleFunctionType(StringView &MangledName, bool HasThisQuals)
{
...
CustomTypeNode *
Demangler::demangleCustomType(StringView &MangledName)
{
    assert(MangledName.startsWith('?'));
...
PrimitiveTypeNode *
Demangler::demanglePrimitiveType(StringView &MangledName)
{
...
TagTypeNode *
Demangler::demangleClassType(StringView &MangledName)
{
...
PointerTypeNode *
Demangler::demanglePointerType(StringView &MangledName)
{
...
Qualifiers
Demangler::demanglePointerExtQualifiers(StringView &MangledName)
{
    Qualifiers Quals = Q_None;
...
ArrayTypeNode *
Demangler::demangleArrayType(StringView &MangledName)
{
...
// RetDec {
Demangler::Demangler(unknown::ms_demangle::ArenaAllocator &allocator) : Arena(allocator), Backrefs() {}
// } RetDec

char *
unknown::microsoftDemangle(const char *MangledName, char *Buf, size_t *N, int *Status, MSDemangleFlags Flags)
{
