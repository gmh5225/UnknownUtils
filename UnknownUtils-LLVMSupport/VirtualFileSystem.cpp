...
#include "unknown/Support/VirtualFileSystem.h"
...
#include "unknown/Support/Casting.h"
#include "unknown/Support/Chrono.h"
#include "unknown/Support/Compiler.h"
#include "unknown/Support/Debug.h"
#include "unknown/Support/Errc.h"
#include "unknown/Support/ErrorHandling.h"
#include "unknown/Support/ErrorOr.h"
#include "unknown/Support/FileSystem.h"
#include "unknown/Support/MemoryBuffer.h"
#include "unknown/Support/Path.h"
#include "unknown/Support/Process.h"
#include "unknown/Support/SMLoc.h"
#include "unknown/Support/SourceMgr.h"
#include "unknown/Support/YAMLParser.h"
...
using namespace unknown::vfs;

using unknown::sys::fs::file_status;
using unknown::sys::fs::file_type;
using unknown::sys::fs::perms;
using unknown::sys::fs::UniqueID;
...
FileSystem::getBufferForFile(const unknown::Twine &Name, int64_t FileSize,
...
std::error_code FileSystem::makeAbsolute(SmallVectorImpl<char> &Path) const {
  if (unknown::sys::path::is_absolute(Path))
    return {};
  ...
  unknown::sys::fs::make_absolute(WorkingDir.get(), Path);
  return {};
}
...
static bool pathHasTraversal(StringRef Path) {
  ...
  for (StringRef Comp : unknown::make_range(path::begin(Path), path::end(Path)))
...
class RealFile : public File {
  ...
  RealFile(int FD, StringRef NewName, StringRef NewRealPathName)
      : FD(FD), S(NewName, {}, {}, {}, {}, {},
                  unknown::sys::fs::file_type::status_error, {}),
...
unknown::ErrorOr<std::string> RealFileSystem::getCurrentWorkingDirectory() const {
  std::lock_guard<std::mutex> Lock(CWDMutex);
  if (!CWDCache.empty())
    return CWDCache;
  SmallString<256> Dir;
  if (std::error_code EC = unknown::sys::fs::current_path(Dir))
    return EC;
  CWDCache = Dir.str();
  return CWDCache;
}
...
std::error_code RealFileSystem::setCurrentWorkingDirectory(const Twine &Path) {
  ...
  if (auto EC = unknown::sys::fs::set_current_path(Path))
    return EC;
...
std::error_code RealFileSystem::isLocal(const Twine &Path, bool &Result) {
  return unknown::sys::fs::is_local(Path, Result);
}

std::error_code
RealFileSystem::getRealPath(const Twine &Path,
                            SmallVectorImpl<char> &Output) const {
  return unknown::sys::fs::real_path(Path, Output);
}
...
class RealFSDirIter : public unknown::vfs::detail::DirIterImpl {
  unknown::sys::fs::directory_iterator Iter;

public:
  RealFSDirIter(const Twine &Path, std::error_code &EC) : Iter(Path, EC) {
    if (Iter != unknown::sys::fs::directory_iterator())
  ...
  std::error_code increment() override {
    std::error_code EC;
    Iter.increment(EC);
    CurrentEntry = (Iter == unknown::sys::fs::directory_iterator())
                       ? directory_entry()
...
ErrorOr<Status> OverlayFileSystem::status(const Twine &Path) {
  ...
  for (iterator I = overlays_begin(), E = overlays_end(); I != E; ++I) {
    ...
    if (Status || Status.getError() != unknown::errc::no_such_file_or_directory)
      return Status;
  }
  return make_error_code(unknown::errc::no_such_file_or_directory);
}

ErrorOr<std::unique_ptr<File>>
OverlayFileSystem::openFileForRead(const unknown::Twine &Path) {
  // FIXME: handle symlinks that cross file systems
  for (iterator I = overlays_begin(), E = overlays_end(); I != E; ++I) {
    auto Result = (*I)->openFileForRead(Path);
    if (Result || Result.getError() != unknown::errc::no_such_file_or_directory)
      return Result;
  }
  return make_error_code(unknown::errc::no_such_file_or_directory);
}

unknown::ErrorOr<std::string>
...
unknown::vfs::detail::DirIterImpl::~DirIterImpl() = default;

namespace {

class OverlayFSDirIterImpl : public unknown::vfs::detail::DirIterImpl {
  OverlayFileSystem &Overlays;
  std::string Path;
  OverlayFileSystem::iterator CurrentFS;
  directory_iterator CurrentDirIter;
  unknown::StringSet<> SeenNames;
  ...
  std::error_code incrementImpl(bool IsFirstTime) {
    while (true) {
      ...
      StringRef Name = unknown::sys::path::filename(CurrentEntry.path());
...
public:
  InMemoryNode(unknown::StringRef FileName, InMemoryNodeKind Kind)
      : Kind(Kind), FileName(unknown::sys::path::filename(FileName)) {}
...
public:
  InMemoryFile(Status Stat, std::unique_ptr<unknown::MemoryBuffer> Buffer)
  ...
  unknown::MemoryBuffer *getBuffer() const { return Buffer.get(); }
...
public:
  ...
  unknown::ErrorOr<Status> status() override {
  ...
  getBuffer(const Twine &Name, int64_t FileSize, bool RequiresNullTerminator,
            bool IsVolatile) override {
    unknown::MemoryBuffer *Buf = Node.getBuffer();
    return unknown::MemoryBuffer::getMemBuffer(
...
class InMemoryDirectory : public InMemoryNode {
  Status Stat;
  unknown::StringMap<std::unique_ptr<InMemoryNode>> Entries;
...
InMemoryFileSystem::InMemoryFileSystem(bool UseNormalizedPaths)
    : Root(new detail::InMemoryDirectory(
          Status("", getNextVirtualUniqueID(), unknown::sys::TimePoint<>(), 0, 0,
                 0, unknown::sys::fs::file_type::directory_file,
                 unknown::sys::fs::perms::all_all))),
...
bool InMemoryFileSystem::addFile(const Twine &P, time_t ModificationTime,
  ...
  if (useNormalizedPaths())
    unknown::sys::path::remove_dots(Path, /*remove_dot_dot=*/true);

  if (Path.empty())
    return false;

  detail::InMemoryDirectory *Dir = Root.get();
  auto I = unknown::sys::path::begin(Path), E = sys::path::end(Path);
  ...
  while (true) {
    ...
    if (!Node) {
      if (I == E) {
        ...
        if (HardLinkTarget)
          ...
          Status Stat(P.str(), getNextVirtualUniqueID(),
                      unknown::sys::toTimePoint(ModificationTime), ResolvedUser,
      ...
      Status Stat(
          ...
          getNextVirtualUniqueID(), unknown::sys::toTimePoint(ModificationTime),
          ResolvedUser, ResolvedGroup, 0, sys::fs::file_type::directory_file,
          NewDirectoryPerms);
      Dir = cast<detail::InMemoryDirectory>(Dir->addChild(
          Name, unknown::make_unique<detail::InMemoryDirectory>(std::move(Stat))));
      continue;
    }
...
bool InMemoryFileSystem::addFileNoOwn(const Twine &P, time_t ModificationTime,
  ...
  return addFile(P, ModificationTime,
                 unknown::MemoryBuffer::getMemBuffer(
...
lookupInMemoryNode(const InMemoryFileSystem &FS, detail::InMemoryDirectory *Dir,
  ...
  auto I = unknown::sys::path::begin(Path), E = unknown::sys::path::end(Path);
  while (true) {
...
unknown::ErrorOr<Status> InMemoryFileSystem::status(const Twine &Path) {
...
InMemoryFileSystem::openFileForRead(const Twine &Path) {
  ...
  return make_error_code(unknown::errc::invalid_argument);
}

namespace {

/// Adaptor from InMemoryDir::iterator to directory_iterator.
class InMemoryDirIterator : public unknown::vfs::detail::DirIterImpl {
  ...
  void setCurrentEntry() {
    if (I != E) {
      ...
      unknown::sys::path::append(Path, I->second->getFileName());
...
directory_iterator InMemoryFileSystem::dir_begin(const Twine &Dir,
  ...
  EC = make_error_code(unknown::errc::not_a_directory);
  return directory_iterator(std::make_shared<InMemoryDirIterator>());
}
...
InMemoryFileSystem::getRealPath(const Twine &Path,
  ...
  if (auto EC = makeAbsolute(Output))
    return EC;
  unknown::sys::path::remove_dots(Output, /*remove_dot_dot=*/true);
  return {};
}
...
class unknown::vfs::VFSFromYamlDirIterImpl
    : public unknown::vfs::detail::DirIterImpl {
  std::string Dir;
...
RedirectingFileSystem::dumpEntry(RedirectingFileSystem::Entry *E,
  ...
  if (E->getKind() == RedirectingFileSystem::EK_Directory) {
    ...
    for (std::unique_ptr<Entry> &SubEntry :
         unknown::make_range(DE->contents_begin(), DE->contents_end()))
...
class unknown::vfs::RedirectingFileSystemParser {
  yaml::Stream &Stream;
  ...
  lookupOrCreateEntry(RedirectingFileSystem *FS, StringRef Name,
    ...
    std::unique_ptr<RedirectingFileSystem::Entry> E =
        unknown::make_unique<RedirectingFileSystem::RedirectingDirectoryEntry>(
  ...
  void uniqueOverlayTree(RedirectingFileSystem *FS,
    ...
    case RedirectingFileSystem::EK_File: {
      ...
      auto *DE = dyn_cast<RedirectingFileSystem::RedirectingDirectoryEntry>(
          NewParentE);
      DE->addContent(
          unknown::make_unique<RedirectingFileSystem::RedirectingFileEntry>(
  ...
  parseEntry(yaml::Node *N, RedirectingFileSystem *FS, bool IsRootEntry) {
    ...
    for (auto &I : *M) {
      ...
      if (Key == "name") {
        ...
        if (FS->IsRelativeOverlay) {
          ...
          unknown::sys::path::append(FullPath, Value);
        } else {
          FullPath = Value;
        }
    ...
    switch (Kind) {
    case RedirectingFileSystem::EK_File:
      Result = unknown::make_unique<RedirectingFileSystem::RedirectingFileEntry>(
...
RedirectingFileSystem::create(std::unique_ptr<MemoryBuffer> Buffer,
  ...
  if (!YAMLFilePath.empty()) {
    ...
    std::error_code EC = unknown::sys::fs::make_absolute(OverlayAbsDir);
...
RedirectingFileSystem::lookupPath(sys::path::const_iterator Start,
  ...
  if (!DE)
    return make_error_code(unknown::errc::not_a_directory);
...
ErrorOr<Status> RedirectingFileSystem::status(const Twine &Path) {
  ...
  if (!Result) {
    if (IsFallthrough &&
        Result.getError() == unknown::errc::no_such_file_or_directory) {
...
public:
  ...
  ErrorOr<std::unique_ptr<unknown::MemoryBuffer>>
...
RedirectingFileSystem::openFileForRead(const Twine &Path) {
  ...
  if (!E) {
    if (IsFallthrough &&
        E.getError() == unknown::errc::no_such_file_or_directory) {
  ...
  Status S = getRedirectedFileStatus(Path, F->useExternalName(UseExternalNames),
                                     *ExternalStatus);
  return std::unique_ptr<File>(
      unknown::make_unique<FileWithFixedStatus>(std::move(*Result), S));
}
...
static void getVFSEntries(RedirectingFileSystem::Entry *SrcE,
  ...
  for (auto &Comp : Path)
    unknown::sys::path::append(VPath, Comp);
...
public:
  JSONWriter(unknown::raw_ostream &OS) : OS(OS) {}
...
void JSONWriter::startDirectory(StringRef Path) {
  ...
  OS.indent(Indent + 2) << "'name': \"" << unknown::yaml::escape(Name) << "\",\n";
...
void JSONWriter::writeEntry(StringRef VPath, StringRef RPath) {
  ...
  OS.indent(Indent + 2) << "'name': \"" << unknown::yaml::escape(VPath) << "\",\n";
  OS.indent(Indent + 2) << "'external-contents': \""
                        << unknown::yaml::escape(RPath) << "\"\n";
  OS.indent(Indent) << "}";
}
...
void YAMLVFSWriter::write(unknown::raw_ostream &OS) {
  unknown::sort(Mappings, [](const YAMLVFSEntry &LHS, const YAMLVFSEntry &RHS) {
...
std::error_code VFSFromYamlDirIterImpl::incrementContent(bool IsFirstTime) {
  ...
  while (Current != End) {
    ...
    unknown::sys::path::append(PathStr, (*Current)->getName());
    sys::fs::file_type Type;
