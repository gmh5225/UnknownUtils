...
#include "unknown/ADT/Statistic.h"
...
#include "unknown/Support/Timer.h"
#include "unknown/Support/YAMLTraits.h"
...
class StatisticInfo {
  ...
  friend void unknown::PrintStatistics();
  friend void unknown::PrintStatistics(raw_ostream &OS);
  friend void unknown::PrintStatisticsJSON(raw_ostream &OS);
...
StatisticInfo::~StatisticInfo() {
  if (::Stats || PrintOnExit)
    unknown::PrintStatistics();
}

void unknown::EnableStatistics(bool PrintOnExit) {
  Enabled = true;
  ::PrintOnExit = PrintOnExit;
}

bool unknown::AreStatisticsEnabled() {
...
void unknown::PrintStatistics(raw_ostream &OS) {
...
void unknown::PrintStatisticsJSON(raw_ostream &OS) {
...
const std::vector<std::pair<StringRef, unsigned>> unknown::GetStatistics() {
  sys::SmartScopedLock<true> Reader(*StatLock);
